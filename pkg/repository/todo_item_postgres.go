...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	createListItemQuery := fmt.Sprintf(`INSET INTO lists_items (list_id, item_id) values ($1, $2)`)
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT * FROM todo_items ti INNER JOIN lists_items li on li.item_id = ti.id INNER JOIN users_lists ul on ul.list_id = li.list_id WHERE li.list_id = $1 AND ul.user_id = $2`)
